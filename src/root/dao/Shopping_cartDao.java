...
import root.po.Shopping_cart;
import root.util.DBUtil;

public class Shopping_cartDao {
	public boolean addGoods(Shopping_cart sc) {
		return DBUtil.executeUpdate("insert into shopping_cart(good_id,good_name,good_count,good_price,u_id) values(?,?,?,?,?)", 
				new Object[]{sc.getGood_id(),sc.getGood_name(),sc.getGood_count(),sc.getGood_price(),sc.getU_id()});
	}


	public List<Map<String, String>> queryAllGoods() {
		return DBUtil.executeQuery("select s.*,u.username from shopping_cart s,user u where s.u_id=u.u_id", null);
	}
